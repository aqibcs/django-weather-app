    ...
    for item in data.get("list", []):
        ...
        if date not in forecast_list:
            forecast_list[date] = {
                ...
                "pressure":
                f"{item['main']['pressure']} hPa",
                "wind_speed":
                f"{item['wind']['speed']} km/h",
                "icon":
...
def weather_view(request):
    ...
    weather_data = parse_weather_data(fetch_weather_data(
        "weather", params)) if params else None
    forecast_data = parse_forecast_data(fetch_weather_data(
        "forecast", params)) if params else None
